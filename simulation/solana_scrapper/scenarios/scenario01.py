...
for tps in [500, 5000]:
    ...
    mean_tps_non_vote = []
    labels = []
    for nodes in [100, 500, 1000, 1500, 2000, 4000, 10_000]:
        scenario01 = Scenario01()
        p = java.SimulatorSubprocess(nodes=nodes, epoch=5, tps=tps).run()
        out.OutputMetadata().parse(scenario01.calculate_epoch_stats)
